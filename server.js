...
const requireAdmin = async (req, res, next) => {
    ...
    if (!adminId) return res.status(401).json({ error: "Acceso no autorizado." });

    if (adminId === "admin_silvina") return next();
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const newUser = { nombre, email, password: hashedPassword, carrito: [], isAdmin: false };
